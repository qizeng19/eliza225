 ...
 */
export function deriveSolanaKeypair(secretPath: string, subject: string): Keypair {
    if (!secretPath || !subject) {
        throw new Error("Secret path and subject are required for key derivation");
    }
...
// 使用示例:
// const keypair = deriveSolanaKeypair("your-secret-salt", "solana");
