...
export async function loadCharacterFromOnchain(): Promise<Character[]> {
    ...
    async function loadLocalCharacters(): Promise<Character[]> {
        // 1. 构建角色配置文件目录路径
        ...
        try {
            ...
            const characters = await Promise.all(
                fileNames.map(fileName => 
                    // 对每个文件使用 loadCharacter 函数加载
                    loadCharacter(path.join(characterDirPath, fileName))
                )
            );
            ...
            // 4. 错误处理：如果目录不存在或读取失败
            elizaLogger.error(`Error loading local characters: ${error}`);
            return []; // 返回空数组作为默认值
        }
    }
...
const startAgents = async () => {
    ...
    if (process.env.IS_IMPORT_LOCAL_CHARACTER === "TRUE") {
        characters = await loadLocalCharacters();
    }
